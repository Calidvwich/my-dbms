...
struct AggregateExpr : public Expr {
    ...
    AggregateExpr(AggFunc func_, std::shared_ptr<Col> col_, bool count_star_, std::string alias_)
        : func(func_), col(std::move(col_)), count_star(count_star_), alias(std::move(alias_)) {}
};
...
struct SelectStmt : public TreeNode {
    ...
    SelectStmt(std::vector<std::shared_ptr<AggregateExpr>> aggregates_,
        ...
        : aggregates(std::move(aggregates_)), tabs(std::move(tabs_)), conds(std::move(conds_)),
          has_sort(false), has_limit(false), limit(-1) {}
};
...
struct SemValue {
    ...
    std::shared_ptr<AggregateExpr> sv_agg;
    std::vector<std::shared_ptr<AggregateExpr>> sv_aggs;
