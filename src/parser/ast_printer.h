...
private:
    ...
    static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
        ...
        if (auto x = std::dynamic_pointer_cast<Help>(node)) {
            ...
            print_val(x->tab_name, offset);
            print_val(x->col_name, offset);
        } else if (auto x = std::dynamic_pointer_cast<AggregateExpr>(node)) {
            static const std::map<AggFunc, std::string> names = {
                {AGG_COUNT, "COUNT"}, {AGG_MAX, "MAX"}, {AGG_MIN, "MIN"}, {AGG_SUM, "SUM"}};
            std::cout << "AGGREGATE\n";
            print_val(names.at(x->func), offset);
            if (x->count_star) {
                print_val("*", offset);
            } else {
