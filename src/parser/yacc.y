...
%token BIGINT DATETIME INVALID_INTEGER COUNT MAX MIN SUM AS
// non-keywords
%token LEQ NEQ GEQ T_EOF

// type-specific tokens
%token <sv_str> IDENTIFIER VALUE_STRING
%token <sv_int64> VALUE_INT
...
%type <sv_int64> opt_limit_clause
...
dml:
    ...
    |   SELECT aggregateList FROM tableList optWhereClause
    {
...
type:
    ...
    |   CHAR '(' VALUE_INT ')'
    {
        if ($3 <= 0 || $3 > std::numeric_limits<int>::max()) {
            YYERROR;
        }
...
aggregateExpr:
    ...
    {
        $$ = std::make_shared<AggregateExpr>(AGG_COUNT, $3, false, $6);
    }
    |   COUNT '(' '*' ')' AS colName
    {
        $$ = std::make_shared<AggregateExpr>(AGG_COUNT, nullptr, true, $6);
    }
    |   MAX '(' col ')' AS colName
    {
        $$ = std::make_shared<AggregateExpr>(AGG_MAX, $3, false, $6);
    }
    |   MIN '(' col ')' AS colName
    {
        $$ = std::make_shared<AggregateExpr>(AGG_MIN, $3, false, $6);
    }
    |   SUM '(' col ')' AS colName
    {
        $$ = std::make_shared<AggregateExpr>(AGG_SUM, $3, false, $6);
    }
    ;
