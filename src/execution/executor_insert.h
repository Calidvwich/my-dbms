...
class InsertExecutor : public AbstractExecutor {
   ...
   public:
    ...
    std::unique_ptr<RmRecord> Next() override {
        ...
        std::vector<IxIndexHandle *> handles;
        ...
        size_t inserted = 0;
        try {
            for (; inserted < handles.size(); inserted++) {
                if (handles[inserted]->insert_entry(
                        keys[inserted].data(), rid_, context_ == nullptr ? nullptr : context_->txn_) == IX_NO_PAGE) {
                    throw UniqueConstraintError();
                }
            }
        } catch (...) {
            while (inserted > 0) {
                inserted--;
                handles[inserted]->delete_entry(
                    keys[inserted].data(), context_ == nullptr ? nullptr : context_->txn_);
            }
