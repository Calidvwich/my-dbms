...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    page_id_t original_page = node->get_page_no();
    bool parent_removed = coalesce(&neighbor, &node, &parent, index, transaction, root_is_latched);
    IxNodeHandle *sibling = neighbor->get_page_no() == original_page ? node : neighbor;
...
bool IxIndexHandle::adjust_root(IxNodeHandle *old_root_node) {
    ...
    // Keep an empty leaf root allocated so a later insert can reuse it.
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    if (!(*neighbor_node)->is_leaf_page()) {
        ...
        if (file_hdr_->first_leaf_ == (*node)->get_page_no()) {
            file_hdr_->first_leaf_ = (*neighbor_node)->get_page_no();
        }
    }

    (*parent)->erase_pair(index);
    if (index == 0 && (*parent)->get_size() > 0) {
        maintain_parent(*parent);
    }
