...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    while (written < num_bytes) {
        ssize_t result = pwrite(fd, offset + written, num_bytes - written, file_offset + written);
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    while (bytes_read < num_bytes) {
        ssize_t result = pread(fd, offset + bytes_read, num_bytes - bytes_read, file_offset + bytes_read);
        if (result < 0 && errno == EINTR) {
            continue;
        }
        if (result <= 0) {
...
int DiskManager::open_file(const std::string &path) {
    ...
    if (fd >= MAX_FD) {
        close(fd);
        throw InternalError("File descriptor exceeds DiskManager::MAX_FD");
    }
    path2fd_[path] = fd;
    fd2path_[fd] = path;
    int file_size = get_file_size(path);
    page_id_t allocated_pages =
        file_size <= 0 ? 0 : static_cast<page_id_t>((file_size + PAGE_SIZE - 1) / PAGE_SIZE);
    fd2pageno_[fd].store(allocated_pages);
    return fd;
}
