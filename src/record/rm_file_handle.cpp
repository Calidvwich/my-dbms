...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (slot_no == file_hdr_.num_records_per_page) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        throw InternalError("No free slot in free record page");
    }
...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
    ...
    if (Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        throw InternalError("Record slot is already occupied");
    }
        ...
        if (file_hdr_.first_free_page_no == page_no) {
            ...
            while (current != RM_NO_PAGE) {
                ...
                if (current_page.page_hdr->next_free_page_no == page_no) {
                    ...
                    buffer_pool_manager_->unpin_page(current_page.page->get_page_id(), true);
                    break;
                }
                int next = current_page.page_hdr->next_free_page_no;
                buffer_pool_manager_->unpin_page(current_page.page->get_page_id(), false);
                current = next;
            }
        }
        page_handle.page_hdr->next_free_page_no = RM_NO_PAGE;
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if (page == nullptr) {
        throw InternalError("Buffer pool has no available frame");
    }
