            ...
            switch(x->tag) {
                case T_select:
                {
                    ...
                    if (auto p = std::dynamic_pointer_cast<ProjectionPlan>(x->subplan_)) {
                        output_cols = p->sel_cols_;
                    } else if (auto aggregate = std::dynamic_pointer_cast<AggregatePlan>(x->subplan_)) {
                        for (const auto &spec : aggregate->aggregates_) {
                            output_cols.push_back(TabCol{"", spec.alias});
                        }
                    }
                    return std::make_shared<PortalStmt>(
                        PORTAL_ONE_SELECT, std::move(output_cols), std::move(root), plan);
                }
